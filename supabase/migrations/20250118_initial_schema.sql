...
CREATE TABLE class_instances (
  ...
  -- Computed UTC times (will be set via trigger instead)
...
-- Function to compute UTC times for class instances
CREATE OR REPLACE FUNCTION compute_class_utc_times()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  -- Compute start UTC time
  NEW.start_datetime_utc = (
    NEW.scheduled_date + COALESCE(NEW.custom_start_time, schedule_record.start_time)
  )::timestamp AT TIME ZONE schedule_record.timezone;
  
  -- Compute end UTC time
  NEW.end_datetime_utc = (
    NEW.scheduled_date + COALESCE(NEW.custom_end_time, schedule_record.end_time)
  )::timestamp AT TIME ZONE schedule_record.timezone;
...
-- Apply trigger to compute UTC times
CREATE TRIGGER set_class_utc_times
  BEFORE INSERT OR UPDATE ON class_instances
  FOR EACH ROW
  EXECUTE FUNCTION compute_class_utc_times();
...
BEGIN
  ...
  SELECT * INTO schedule_record FROM schedules WHERE id = p_schedule_id;
  ...
  WHILE curr_date <= p_end_date LOOP
    ...
    IF EXTRACT(DOW FROM curr_date) = schedule_record.day_of_week THEN
      -- Insert if not exists
      INSERT INTO class_instances (schedule_id, scheduled_date)
      VALUES (p_schedule_id, curr_date)
